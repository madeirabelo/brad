...
const EUROPEAN_COUNTRIES = [
  'GB', 'PT', 'ES', 'FR', 'NL', 'GR', 'IT', 'RU', 'DE', 'IE', 'BE', 
  'CH', 'AT', 'SE', 'NO', 'DK', 'FI', 'PL', 'CZ', 'SK', 'HU', 'RO', 
  'BG', 'HR', 'RS', 'SI', 'ME', 'MK', 'AL', 'BA', 'MD', 'UA', 'BY', 
  'LT', 'LV', 'EE', 'IS', 'MT', 'CY', 'LU', 'LI', 'MC', 'SM', 'VA', 
  'AD', 'GI'
];
...
const EuropeMap = () => {
      ...
      try {
        ...
        const europeanData = {
          ...
          features: data.features.filter(f => {
            ...
            const isEuropean = countryCode && EUROPEAN_COUNTRIES.includes(countryCode);
            if (isEuropean) {
              console.log(`Found European country: ${countryName} (${countryCode})`);
            }
            return isEuropean;
          })
        };

        console.log('Filtered European countries:', europeanData.features.map(f => ({
          name: f.properties.name,
          code: countryToISO[f.properties.name]
        })));
        
        if (europeanData.features.length === 0) {
          console.error('No European countries found. Available countries:', 
            data.features.map(f => ({
              name: f.properties.name,
              code: countryToISO[f.properties.name]
            }))
  ...
  useEffect(() => {
    ...
    const renderMap = () => {
      try {
        ...
        const container = document.querySelector('.europe-map-container');
        if (!container) return;
  ...
  return (
    <div className="europe-map-container">
      <div className="map-title">Europe</div>
    </div>
  );
};
